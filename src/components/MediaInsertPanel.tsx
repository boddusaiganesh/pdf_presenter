...
export default function MediaInsertPanel({ onClose, insertAfterIndex }: MediaInsertPanelProps) {
  ...
  const handleURLChange = (value: string) => {
    setUrl(value);
    if (value.trim()) {
      const d = detectMediaType(value.trim());
      setDetected(d);
    } else {
      setDetected(null);
    }
  ...
  const handleInsertURL = () => {
    ...
    addSlide({
      ...
      mediaUrl: url.trim(),
      mediaType: detected.type as any,
    }, afterIndex);
        ...
        <div className="flex items-start gap-3 max-w-xs">
          ...
          <div>
            ...
            <p className="text-xs text-gray-600 mt-0.5">
              To share audio from this video, enable "Share audio" in your meeting app (Google Meet: Share screen → Also share audio checkbox; Zoom: Share Screen → Share computer audio).
            </p>
          ...
          {activeTab === 'url' && (
            <div>
                      ...
                      {isStreamingService(detected.type) && (
                        <div className="mt-2 flex items-start gap-1.5">
                          ...
                          <p className="text-amber-400/80 text-xs">
                            Streaming video gets re-encoded twice (once by the service, once by Meet/Zoom). Consider downloading locally for better quality.
                          </p>
                        </div>
                      )}
                    </div>
              ...
              <div className="mt-4 flex gap-3">
                <button onClick={onClose} className="flex-1 py-2.5 rounded-xl border border-white/[0.1] text-white/60 hover:text-white hover:border-white/20 text-sm transition-all">
                  Cancel
                </button>
                <button
          ...
          {activeTab === 'blank' && (
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  onClick={() => {
                    addSlide({ type: 'blank-white' }, afterIndex);
                    toast.success('Blank white slide added');
                    onClose();
                  }}
                ...
                <button
                  onClick={() => {
                    addSlide({ type: 'blank-black' }, afterIndex);
                    toast.success('Blank black slide added');
                    onClose();
                  }}
