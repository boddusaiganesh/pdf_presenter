...
import {
  ...
  Eye, Shield, ChevronRight, Check, RotateCcw
} from 'lucide-react';
import { useStore, DEFAULT_SETTINGS, STANDARD_COLORS, HIGH_CONTRAST_COLORS } from '../store/useStore';
            ...
            <div className="py-3">
              ...
              <div className="flex flex-wrap gap-2">
                {STANDARD_COLORS.map((c) => (
                  <button
                    key={c}
                    onClick={() => updateSettings({ defaultAnnotationColor: c })}
        ...
        return (
          <div>
            <Toggle
              ...
              label="Confirm Before Deleting Slides"
              description="Show a confirmation dialog before deleting"
            />
            <Toggle
              checked={settings.autoCloseSidePanel}
              onChange={(v) => updateSettings({ autoCloseSidePanel: v })}
              label="Auto-close Side Panel on Present"
              description="Automatically closes the panel when entering present mode"
            />
            <Toggle
              checked={settings.audioRoutingReminder}
              onChange={(v) => updateSettings({ audioRoutingReminder: v })}
              label="Audio Routing Reminder"
              description="Show a reminder about sharing audio when inserting media slides"
            />
            <SliderRow
            ...
            <SliderRow
              ...
              onChange={(v) => updateSettings({ autoAdvanceSeconds: v })}
              description="0 = manual navigation only"
            />
            <SliderRow
              label="Preload Slides Ahead"
              value={settings.preloadSlides}
              min={1} max={20}
              onChange={(v) => updateSettings({ preloadSlides: v })}
              description="How many slides to pre-render in advance"
            />
          </div>
        );
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-white/50 text-xs mt-1 leading-relaxed">
                      ...
                      The slides still look great for remote viewers since Meet/Zoom re-encodes everything anyway.
                    </p>
                  </div>
                </div>
              </div>
        ...
        return (
          <div>
            <div className="rounded-xl bg-indigo-500/10 border border-indigo-500/20 p-4 mb-4">
              <div className="flex items-start gap-2">
                ...
                <p className="text-white/60 text-xs leading-relaxed">
                  These settings are specifically designed for remote teaching. Each option compensates for how Google Meet, Zoom, and Teams re-encode your shared screen.
                </p>
              </div>
            </div>

            <Toggle
              checked={settings.compressionContrastBoost}
              onChange={(v) => updateSettings({ compressionContrastBoost: v })}
              label="Compression-Aware Contrast Boost"
              description="Increases contrast slightly to compensate for video-call compression that softens thin lines and light text"
            />
            {settings.compressionContrastBoost && (
              <SliderRow
                label="Contrast Boost Strength"
                value={settings.contrastBoostStrength}
                min={5} max={50} unit="%"
                onChange={(v) => updateSettings({ contrastBoostStrength: v })}
              />
            )}
            <Toggle
              checked={settings.minBrushSizeFloor}
              onChange={(v) => updateSettings({ minBrushSizeFloor: v })}
              label="Minimum Brush Size Floor"
              description="Warns if brush is set too thin to survive video compression (typically below 3px)"
            />
            {settings.minBrushSizeFloor && (
              <SliderRow
                label="Minimum Threshold"
                value={settings.minBrushSizeThreshold}
                min={1} max={8} unit="px"
                onChange={(v) => updateSettings({ minBrushSizeThreshold: v })}
              />
            )}
            <Toggle
              checked={settings.highContrastPresets}
              onChange={(v) => updateSettings({ highContrastPresets: v })}
              label="High-Contrast Color Presets"
              description="Pre-tested colors that stay legible after Meet/Zoom compression (avoids pale yellows, light grays)"
            />
            <Toggle
              checked={settings.audioRoutingReminder}
              onChange={(v) => updateSettings({ audioRoutingReminder: v })}
              label="Audio Routing Reminder"
              description="Shows a reminder to enable 'Share Audio' in your meeting app when inserting video slides"
            />
            ...
            <div className="py-4 mt-2">
              ...
              <SliderRow
                label="Laser Size"
                value={settings.laserSize}
                min={6} max={32} unit="px"
                onChange={(v) => updateSettings({ laserSize: v })}
              />
              <SliderRow
                label="Glow Intensity"
                value={settings.laserGlow}
                min={0} max={100} unit="%"
                onChange={(v) => updateSettings({ laserGlow: v })}
              />
              <Toggle
                checked={settings.laserTrail}
                onChange={(v) => updateSettings({ laserTrail: v })}
                label="Pointer Trail Effect"
                description="Shows a fading trail behind the laser pointer"
              />
              {settings.laserTrail && (
                <SelectRow
                  label="Trail Length"
                  value={settings.laserTrailLength}
                  options={[
            ...
            <div className="py-4">
              ...
              <SliderRow
                ...
                min={80} max={400} unit="px"
                onChange={(v) => updateSettings({ spotlightSize: v })}
              />
              <SliderRow
                label="Dim Level"
                value={settings.spotlightDimLevel}
                min={30} max={95} unit="%"
                onChange={(v) => updateSettings({ spotlightDimLevel: v })}
                description="How dark the non-spotlighted area gets"
              />
              <Toggle
                checked={settings.spotlightSoftEdge}
                onChange={(v) => updateSettings({ spotlightSoftEdge: v })}
                label="Soft Edge"
                description="Smooth gradient edge on spotlight (vs hard cutoff)"
              />
            </div>
          </div>
        );
        ...
        return (
          <div>
            <div className="rounded-xl bg-white/[0.03] border border-white/[0.07] overflow-hidden mb-4">
              {Object.entries(settings.keyboardShortcuts).map(([action, shortcut], i) => (
                <div key={action} className={cn('flex items-center justify-between px-4 py-3', i > 0 ? 'border-t border-white/[0.05]' : '')}>
                  ...
                  <kbd className="px-2 py-0.5 text-xs font-mono bg-white/[0.08] border border-white/[0.1] rounded text-white/70">
                    {shortcut}
                  </kbd>
                </div>
              ))}
            ...
            <p className="text-white/30 text-xs">
              Shortcut customization coming in a future update. All shortcuts use standard Ctrl/Cmd key combinations for cross-platform compatibility.
            </p>
          </div>
        );
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-xl bg-white/[0.03] border border-white/[0.07] p-4 space-y-3">
              {[
                ...
                ['Architecture', 'Single-window, screen-share-first design'],
            ...
            <div className="rounded-xl bg-emerald-500/10 border border-emerald-500/20 p-4">
              <div className="flex items-start gap-2">
                ...
                <p className="text-white/60 text-xs leading-relaxed">
                  Every feature in this application was designed with one question in mind: 
      ...
      <div className="relative w-full max-w-3xl bg-[#13151f] border border-white/[0.08] rounded-2xl shadow-2xl shadow-black/50 overflow-hidden animate-slide-up flex flex-col max-h-[85vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/[0.06]">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={handleReset}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-white/40 hover:text-white/70 hover:bg-white/[0.05] text-xs transition-all"
            >
